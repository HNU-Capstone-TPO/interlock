...
price = int
color = ["빨강", "파랑", "주황", "보라", "분홍", "갈색",
         "노랑", "초록", "하양", "검정", "회색", "아이보리", "색깔테스트"]
red = ["붉은색", "장미색", "적색", "red", "자주색", "세리스", "자주빛", "주홍색",
       "빨강색", "빨간색", "진홍색", "크림슨", "버건디", "다홍색", "스칼렛", "레드벨벳"]
orange = ["주황색", "orange", "오렌지색", "귤색",]
yellow = ["노란색", "유황색", "노랑색", "레몬색", "yellow", "샛노란색", "샛노랑색", "개나리색"]
green = ["초록색", "카키색", "연녹색", "풀색", "green", "mint",
         "민트색", "녹색", "올리브색", "라임색", "연두색", "춘록색"]
blue = ["파랑색", "담청색", "파란색", "청색", "연청색", "하늘색",
        "blue", "푸른색", "청록색", "남색", "코발트블루", "군청색", "바다색"]
purple = ["보라색", "purple"]
pink = ["분홍색", "핑크색", "pink"]
white = ["흰색", "하양색", "화이트", "white", "하얀색", "백색"]
black = ["검정색", "검색", "까만색", "검은색", "흑색", "고동색", "black"]
brown = ["갈색", "황갈색", "탠", "황토색", "금색", "누런색", "누렁색", "브라운",
         "brown", "beige", "베이지", "연한베이지", "연갈색", "다크브라운", "흑갈색"]
...
def createform(request):
    ...
    if input8:
        ...
        for q in input8:
            ...
            matched_users += users
        matched_users = [user for user in matched_users if any(
            q in user.tag for q in input8)]
    ...
    for rdb_item in rdb_result:
        ...
        dataresult = serializer1.data + \
            serializer2.data + serializer3.data + \
    ...
    datanewbie_total1 = remove_duplicates(
        datanewbie_total1, key=lambda x: x['id'])

    dataexpert_total1 = datasearch + dataexpert_total
    dataexpert_total1 = remove_duplicates(
        dataexpert_total1, key=lambda x: x['id'])
    ...
    if button_value == 'E':   # expert
        # dataexpert
        return render(request, 'folder/searchreal.html', {'users': dataexpert_total1})
    ...
    if request.method == 'POST':
        ...
        for rdb_item in rdb_result:
            ...
            users2 = User.objects.filter(
                id__in=names, part__icontains="하의")[:1]
            users3 = User.objects.filter(
                id__in=names, part__icontains="신발")[:1]
            users4 = User.objects.filter(
                id__in=names, part__icontains="모자")[:1]
            users5 = User.objects.filter(
                id__in=names, part__icontains="아우터")[:1]
            users6 = User.objects.filter(
                id__in=names, part__icontains="포인트")[:1]
            if gdbsearch_part:
                if gdbsearch_part == "상의":
                    users1 = []
                elif gdbsearch_part == "하의":
        ...
        n_part_counts = {'상의': 0, '하의': 0,
                         '신발': 0, '모자': 0, '아우터': 0, '포인트': 0}
        e_part_counts = {'상의': 0, '하의': 0,
                         '신발': 0, '모자': 0, '아우터': 0, '포인트': 0}
        for data in dataresult_total:
            data_part = data['part']
            if n_part_counts[data_part] < 3:  # 각 부위별로 최대 3개까지 선택
